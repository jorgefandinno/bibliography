...
def name_info_to_str(name_info):
    '''
    Concatenate the name information into a string.
    '''
    # print("name_info", name_info)
    first = " ".join(name_info['first'])
    von = " ".join(name_info['von'])
    last = " ".join(name_info['last'])
    jr = " ".join(name_info['jr'])
    previous = first != ""
    if previous and von:
        von = f" {von}"
...
def format_people(x):
    ...
    for name in x.split(' and '):
        if name in special_cases.WHOLE_NAMES:
            people_names.append(name)
            continue
        name_info = splitname(name)
        if "first" in name_info:
            first_name = " ".join(name_info["first"])
            name_info_tuple = (first_name, " ".join(name_info["von"]), " ".join(name_info["last"]), " ".join(name_info["jr"]))
            if name_info_tuple not in special_cases.SPLITED_NAMES:
                formated_first_name = format_first_name(first_name)
                name_info["first"] = [formated_first_name]
                if first_name != formated_first_name:
                    modified_people.add(name_info_tuple)
        formated_name = name_info_to_str(name_info)
        people_names.append(formated_name)
    y = ' and '.join(people_names)
    return y
...
def run():
    ...
    if res.command == "format":
        ...
        # format_bib('small.bib')
        return 0
    
    if res.command == "special-names":
